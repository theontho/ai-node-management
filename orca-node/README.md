# Reproducible Orca Node

This directory defines a one-command Docker Compose appliance for an AMD64
Ubuntu 24.04 node. Tailscale runs once on the host as the shared maintenance
and service network; the Compose appliance contains only the headless Orca
runtime.
...
Compose publishes TCP 6768 only on the host's current Tailscale IPv4 address.
Orca is not published on the LAN address. The container drops all Linux
capabilities and runs as UID/GID 1000; it has no TUN device, network
capabilities, Tailscale key, or Tailscale state.
...
Orca does not currently publish an official OCI image on Docker Hub, GitHub
Container Registry, or its GitHub releases. The official repository provides
a headless Linux guide and Dockerfiles for its own container tests, but its
supported release artifacts are AppImage, Debian, and RPM packages.
...
## Host Tailscale prerequisite

The Linux unattended installer installs and enrolls Tailscale on the host.
Other Ubuntu hosts must provide an active `tailscaled.service` with
`BackendState=Running` before Orca deployment. The deployer reads
`tailscale ip -4`, binds Orca only to that address, and stores no Tailscale
credentials or identity in the Compose appliance.
...
```bash
./deploy.sh \
  --host node-admin@lin-lunar-maple.local \
  --environment-name lin-lunar-maple
```

Use the actual hostname generated by the Linux installer for `--host` and
`--environment-name`. It is shown on the node's physical console and advertised
through mDNS.
...
starts the appliance, waits for the container to become healthy, discovers the
host's tailnet IPv4 address unless `--pairing-address` was supplied, extracts
Orca's pairing code, saves the remote environment in the local Orca app, checks
remote runtime status, and registers an NVMe-backed `scratch` project for
immediate use from desktop or mobile. It also gives the container a stable
`HOSTNAME-orca` name, so terminal prompts identify both the node and the
container instead of showing a transient Docker ID.
...
Existing sidecar deployments can retain their tailnet IP and saved Orca
pairing:

```bash
./migrate-sidecar-to-host.sh \
  --host ai-controller@asus-linux-book.local \
  --environment-name asus-linux-book
```

Install the official host Tailscale package first, but do not enroll it or
create host state. The migration stops the old appliance, transfers the
sidecar identity to the host service, replaces Compose and systemd definitions,
verifies Orca through the same tailnet address, then deletes the obsolete
sidecar auth key and state. It restores the old sidecar configuration if the
cutover fails.
